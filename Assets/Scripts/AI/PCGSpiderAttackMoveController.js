...
function Awake () {
	...
	ai = transform.parent.GetComponentInChildren.<PCGSpiderAI> ();
...
function Update () {
	...
	// Every few seconds do a ray test to see if the player can be seen
	// If they havent been seen in a while, go into LostTrack mode
...
function Explode () {
	...
	if (targetHealth) {
		...
		targetHealth.playerDmgFrom[FEATURES.Spider] += damageAmount;
	}
