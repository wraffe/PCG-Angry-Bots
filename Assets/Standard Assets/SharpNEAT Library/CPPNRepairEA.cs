...
public class CPPNRepairEA {
	...
	public void Initialize(PCGSharpHighLevelFeatures originalFeatures) {
	...
	public void RunEA(List<PCGNeatNodeData> geometryData, PCGWekaClassifier updatedPlayerModel) {
		...
		if (contentEA == null)
			...
			if (geometryData == null)
				Debug.Log("RunEA: Geometry data null");
			else
				Debug.Log("RunEA: Geometry data all good");
			((CPPNRepairExperiment)(experiment)).geomNodeList = geometryData;
			((CPPNRepairExperiment)(experiment)).playerModel = updatedPlayerModel;
	...
	public void PrintStats() {
		...
		for (int i = 0; i < (experiment as CPPNRepairExperiment).geomNodeList.Count; i++) {
			// Get cppn output for each node
			box.InputSignalArray[0] = (experiment as CPPNRepairExperiment).geomNodeList[i].normDepth;
			box.InputSignalArray[1] = (experiment as CPPNRepairExperiment).geomNodeList[i].normSiblingIndex;
			...
			Debug.Log("(" + (experiment as CPPNRepairExperiment).geomNodeList[i].normDepth + ","
				+ (experiment as CPPNRepairExperiment).geomNodeList[i].normSiblingIndex + ") -> (" + outputString + ")");			
		...
		if ((experiment as CPPNRepairExperiment).playerModel != null) {
			double[] distributions = (experiment as CPPNRepairExperiment).playerModel.ClassifyNewData(featureCounts.ToDoubleArray());
			Debug.Log("Classifier: Champ Distributions: Dislike = " + distributions[0] + ", Like = " + distributions[1]);
			(experiment as CPPNRepairExperiment).playerModel.PrintClassifierTestReport();
		}
	...
	public void TestSharpHighLevelFeatures() {
		...
		PCGSharpHighLevelFeatures featureCounts = new PCGSharpHighLevelFeatures();
		for (int i = 0; i < ((CPPNRepairExperiment)experiment).geomNodeList.Count; i++) {			
			// Get cppn output for each node
			PCGNeatNodeData currentNode = ((CPPNRepairExperiment)experiment).geomNodeList[i];
